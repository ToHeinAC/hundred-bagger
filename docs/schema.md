...
**`portfolio`** (Phase 4) — one row per position, `status` ∈ `open` \| `closed`. `entry_roic` is snapshotted at entry so that thesis drift (entry ROIC vs current ROIC) is measurable without reconstructing history. `realized_return_pct` is written on close. `currency` (`USD` default, `EUR`) is the currency of `entry_price` and of the exchange the ticker is quoted on — it is what lets a mixed book be stated in one currency ([portfolio.md](portfolio.md#one-currency)). It arrived after the table did, so `schema.sql` carries an `ALTER TABLE … ADD COLUMN IF NOT EXISTS` beneath the DDL; pre-existing rows default to `USD`.
